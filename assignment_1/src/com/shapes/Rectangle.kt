...
class Rectangle(private val point1: Point, private val point2: Point) {
    ...
    private val width = if (abs(this.point1.getX() - this.point2.getX()) != 0.0) {
        abs(this.point1.getX() - this.point2.getX())
    } else {
        throw Exception("Invalid width, must be > 0")
    }

    private val height = if (abs(this.point1.getY() - this.point2.getY()) != 0.0) {
        abs(this.point1.getY() - this.point2.getY())
    } else {
        throw Exception("Invalid height, must be > 0")
    }
